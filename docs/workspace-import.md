...
- `current/` is one atomic pointer to a hash-verified snapshot containing `state.json`, `completion-report.md`, and delivered output files. Preserved publishing edits can be invalid or stale: check `publishing_issues` before treating a file as ready. Read a single resolved snapshot when consuming several files programmatically. Separate path lookups across a concurrent commit may naturally observe different snapshots.
...
- `snapshots/` retains complete and partial committed views. Do not edit history or state. Direct publishing edits remain current through inspection/resume and become distinct immutable historical artifacts with exact hashes and provenance. Explicit selected regeneration can replace them, retaining history; they do not become transcript corrections or authoritative metadata.
- On the next operation, current and original hashes are checked. Damaged generated outputs are removed from the usable set; an invalid timed transcript also supersedes its generated consumers. Publishing edits remain delivered with honest validation/staleness issues. An explicit reimport repairs preserved transcript outputs.
